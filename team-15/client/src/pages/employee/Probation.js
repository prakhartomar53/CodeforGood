...
import "../../styles/Probation.scss";
import moment from "moment";
import Probation from "../../images/Probation.svg";
import InternalLayout from "../../components/layout/inner";
import { useQuery } from "react-query";
import { GetProbatedEmployees } from "../../services/employee.service";
function Applications() {
  const { data: probated } = useQuery("probabted", GetProbatedEmployees);
  console.log(probated);
  return (
    <InternalLayout>
      <div className="probationmain">
        <div className="container">
          <h1 className="heading">Probation</h1>
          <div className="card-row">
            {probated?.map(({ name, email, role, probationEndDate }) => (
              <div className="card">
                <div className="probcard">
                  <div className="top">
                    ...
                    <div className="header-content">
                      ...
                      <span>{name}</span>
                      <br />
                      <span className="font-big">Role: </span>
                      <span>{role}</span>
                      <br />
                      <span className="font-big">Email: </span>
                      <span>{email}</span>
                      <br />
                      <span className="font-big">Left: </span>
                      <span>2 months</span>
                    </div>
                  </div>
                </div>
